 ...
 * 
 * @see com.colin.domain.SysManager
 * @author MyEclipse Persistence Tools
 */

public class SysManagerDAO extends HibernateDaoSupport {
	private static final Logger log = LoggerFactory
			.getLogger(SysManagerDAO.class);
	// property constants
	...
	public SysManager findById(java.lang.Integer id) {
		log.debug("getting SysManager instance with id: " + id);
		try {
			SysManager instance = (SysManager) getHibernateTemplate().get(
					"com.colin.domain.SysManager", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public SysManager merge(SysManager detachedInstance) {
		...
		try {
			SysManager result = (SysManager) getHibernateTemplate().merge(
					detachedInstance);
	...
	public static SysManagerDAO getFromApplicationContext(ApplicationContext ctx) {
		return (SysManagerDAO) ctx.getBean("SysManagerDAO");
	}
}